...
function illShow(){
	// homeContainerというグループをつくる。
	illContainer = new createjs.Container();
	stage.addChild(illContainer);
	illContainer.x = 1024;

	// プリロード
	var preload = new createjs.LoadQueue(false);
	preload.loadManifest(targetUrl[pageCount].imgData);

	// １個ずつの読み込みイベント
	preload.on('fileload',function(e){
		// console.log(e);
		showCanvas(e,illContainer);
	});
	// 全部の読み込み完了
	preload.on('complete',function(e){
		console.log('complete!!!!');
		createjs.Tween.get(illContainer).to({x:0},1000,createjs.Ease.getPowInOut(2));
	});
	// ページカウントを１ふやす
	pageCount++;
}
...
function showCanvas(e,container){
